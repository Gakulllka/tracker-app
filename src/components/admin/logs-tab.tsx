  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select value={filterAction} onChange={(e) => { setFilterAction(e.target.value); setOffset(0); }} className="px-3 py-2 rounded-lg border border-[var(--tracker-border)] text-sm bg-[var(--tracker-bg-card)]">
        ...
        <input type="date" value={filterDateFrom} onChange={(e) => { setFilterDateFrom(e.target.value); setOffset(0); }} className="px-3 py-2 rounded-lg border border-[var(--tracker-border)] text-sm" />
        <span className="text-[var(--tracker-text-muted)] text-sm">—</span>
        <input type="date" value={filterDateTo} onChange={(e) => { setFilterDateTo(e.target.value); setOffset(0); }} className="px-3 py-2 rounded-lg border border-[var(--tracker-border)] text-sm" />
      ...
      <div className="text-xs text-[var(--tracker-text-muted)]">Всего записей: {total}</div>
      ...
      {loading ? (
        ...
        <div className="space-y-1">
          {logs.length === 0 && <div className="text-center py-12 text-[var(--tracker-text-muted)]">Нет записей</div>}
          {logs.map((log) => (
            <div key={log.id} className="border border-[var(--tracker-border)] rounded-lg hover:bg-[var(--tracker-bg-main)] transition-colors">
              <div
                className="flex items-center gap-3 px-4 py-3 cursor-pointer"
                onClick={() => setExpandedId(expandedId === log.id ? null : log.id)}
              >
                <span className="text-xs text-[var(--tracker-text-muted)] whitespace-nowrap w-40">{formatDate(log.createdAt)}</span>
                ...
                <span className="text-sm font-medium text-[var(--tracker-text-main)]">{log.username || "Система"}</span>
                <span className="text-sm text-[var(--tracker-text-muted)] flex-1 truncate">
                  {log.entityType ? `${log.entityType}${log.entityId ? ` #${log.entityId}` : ""}` : ""}
                </span>
                {hasDetails(log) && (expandedId === log.id ? <ChevronUp className="w-4 h-4 text-[var(--tracker-text-muted)]" /> : <ChevronDown className="w-4 h-4 text-[var(--tracker-text-muted)]" />)}
              </div>
              {expandedId === log.id && hasDetails(log) && (
                <div className="px-4 pb-3 pt-0 border-t border-[var(--tracker-border)] bg-[var(--tracker-bg-main)]/50">
                  <div className="grid grid-cols-2 gap-4 mt-2 text-xs">
                    {log.oldValue && (
                      <div>
                        <div className="font-medium text-[var(--tracker-text-muted)] mb-1">Было:</div>
                        <pre className="bg-[var(--tracker-bg-card)] rounded p-2 text-[var(--tracker-text-muted)] overflow-x-auto max-h-32">{(() => { try { return JSON.stringify(JSON.parse(log.oldValue), null, 2); } catch { return log.oldValue; } })()}</pre>
                      </div>
                    )}
                    {log.newValue && (
                      <div>
                        <div className="font-medium text-[var(--tracker-text-muted)] mb-1">Стало:</div>
                        <pre className="bg-[var(--tracker-bg-card)] rounded p-2 text-[var(--tracker-text-muted)] overflow-x-auto max-h-32">{(() => { try { return JSON.stringify(JSON.parse(log.newValue), null, 2); } catch { return log.newValue; } })()}</pre>
                      </div>
                    )}
                    {log.ipAddress && (
                      <div className="col-span-2 text-[var(--tracker-text-muted)]">IP: {log.ipAddress}</div>
                    )}
                  </div>
                </div>
              )}
      ...
      {total > limit && (
        <div className="flex items-center justify-between pt-4">
          <span className="text-sm text-[var(--tracker-text-muted)]">Показано {offset + 1}–{Math.min(offset + limit, total)} из {total}</span>
